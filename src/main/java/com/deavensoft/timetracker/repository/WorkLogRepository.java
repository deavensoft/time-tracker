...
public interface WorkLogRepository extends CrudRepository<WorkLog, Long> {
  ...
  @Query("select e from WorkLog e,User u where e.user.id = ?1 and e.date between ?2 and ?3 order by e.date ")
  List<WorkLog> findWorkLogsBetweenDatesWithUser(Long userId ,LocalDate from, LocalDate to);

  @Query("select e from WorkLog e,Project u where e.project.id = ?1 and e.date between ?2 and ?3 order by e.date ")
  List<WorkLog> findWorkLogsBetweenDatesWithProject(Long projectId ,LocalDate from, LocalDate to);

  @Query("select e from WorkLog e,Project u where e.project.id = ?1 and e.user.id = ?2 and e.date between ?3 and ?4 order by e.date ")
